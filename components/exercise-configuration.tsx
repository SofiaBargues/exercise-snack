...
interface ExerciseConfigurationProps {
  ...
  onComplete: (config: { repetitions: number; intervalMinutes: number }) => void
  onBack: () => void
}

export function ExerciseConfiguration({ selectedExercises, onComplete, onBack }: ExerciseConfigurationProps) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50 py-12">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Configure Your Exercise Routine</h1>
          <p className="text-lg text-gray-600">Set your preferences for repetitions and timing</p>
        </div>

        {/* Selected Exercises Preview */}
        <Card className="p-6 mb-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Your Selected Exercises</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {selectedExercises.map((exercise) => (
              <div key={exercise.id} className="flex items-center gap-3 p-3 bg-orange-50 rounded-lg">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          <Card className="p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Repetitions per Exercise</h3>
            <div className="space-y-4">
              <div className="text-center">
                <span className="text-3xl font-bold text-orange-600">{repetitions}</span>
        ...
        <div className="flex gap-4 justify-center">
          ...
          <Button onClick={handleStart} size="lg" className="bg-orange-600 hover:bg-orange-700 text-white px-8">
            Start Exercise Snacks
          </Button>
        </div>
      </div>
    </div>
